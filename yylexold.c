...
tobinary(st, b) char *st; { /* sub_2892h OK++ */
	int n, c, t;
	char *s;
	n=0;
	s=st;
	...
	switch(c) {
		...
		case 'a': case 'b': case 'c': case 'd': case 'e': case 'f': 
			t = c-'a'; /* bug in original code s/b +10 */ if (b>10) break;
		case 'A': case 'B': case 'C': case 'D': case 'E': case 'F': 
			t = c - 'A'; /* bug in original code s/b +10 */ if (b>10) break;
		default:
			t = -1;
...
yylex() { /* sub_29f6h OK++ */
#ifndef CPM
	int opt_passcom;
	char c;   /* These two used to be local to blocks */
#endif
	static int ifdef=0;
	static char *op2[]={"||",  "&&" , ">>", "<<", ">=", "<=", "!=", "=="};
	...
	char savc, *s; char *skipbl(); int val;
	char **p2;
...
for (;;) {
#ifndef CPM
        opt_passcom = passcom;	/* even with -C option */
